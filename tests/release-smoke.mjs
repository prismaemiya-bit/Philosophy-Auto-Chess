...
async function main() {
  ...
  try {
    ...
    chrome = spawn(chromePath, ["--headless=new", "--disable-gpu", "--mute-audio", "--no-first-run", "--no-default-browser-check", `--remote-debugging-port=${remotePort}`, `--user-data-dir=${userData}`, "--window-size=1920,1080", "about:blank"], { stdio: "ignore" });
    ...
    assert.equal(await evaluate("document.querySelector('.landing h1')?.textContent?.trim()"), "往哲荣耀");
    ...
    assert.ok(saved?.includes('"saveVersion":7'), "wave progress must persist as V7");
    ...
    await waitFor(() => evaluateRestarted("Boolean(document.querySelector('.main-menu'))"), "saved game menu after full restart");
    await waitFor(() => evaluateRestarted(`(() => { const button = [...document.querySelectorAll('.main-menu-actions button')].find((item) => item.textContent.includes('继续征程')); return Boolean(button && Object.keys(button).some((key) => key.startsWith('__reactProps'))); })()`), "continued-game action hydration");
    await evaluateRestarted(`[...document.querySelectorAll('.main-menu-actions button')].find((item) => item.textContent.includes('继续征程')).click()`);
    ...
    assert.equal(afterRestart.saveVersion, 7); assert.deepEqual(afterRestart.pieces, beforeRestart.pieces); assert.equal(afterRestart.wave, beforeRestart.wave);
    console.log(`Release smoke passed (${releaseInfo.displayVersion}, ${releaseInfo.developer}): clean unzip, relative Unicode path, production gates, purchase, deploy, wave, close, restart and V7 continuation.`);
  } catch (error) {
