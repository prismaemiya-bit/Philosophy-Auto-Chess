...
import { capGold, chooseResearch, ECONOMY_RULES, interestForGold, isDeploySlot, isFieldedSlot, isThroneSlot, normalizeProgress, pickShop, settlementIncome, validatePreparationPlan, type BalanceWaveReport, type GameState, type Piece } from "./engine";
import { deploymentPoint, distance, revolutionNodePoint, ROYAL_BARRIER_POINT, routePoint, type Point } from "./positions";
import { MAX_WAVES, waveDefinition } from "./waves";

export { FIXED_STEP_SECONDS, MAX_WAVES };
export type EnemyKind = "swift" | "ordinary" | "armored" | "caster" | "swarm" | "elite" | "cave-boss" | "boss";
export type Lane = SourceRouteId;
export type BossPhaseId = "cave-turn" | "objective-spirit" | "world-night" | "absolute-knowledge";
export type BossPhaseEvent = { id: BossPhaseId; name: string; threshold: number; triggeredAt: number };
export const ABSOLUTE_SPIRIT_PHASES: ReadonlyArray<{ id: BossPhaseId; name: string; threshold: number; description: string }> = [
...
export type BattleSummary = {
  ...
  experienceGained: number; synergyTriggers: Record<string, number>; bossPhases: BossPhaseEvent[]; statistics: WaveStatistics;
};
...
type EnemyTemplate = {
  ...
  attackEvery: number; reward: number; weight: number; className: string; description: string;
};

export const enemyTemplates: Record<EnemyKind, EnemyTemplate> = {
  swift: { name: "快速敌人", glyph: "›", maxHp: 76, speed: .035, coreDamage: 7, attack: 9, attackEvery: 8, reward: 0, weight: 1, className: "swift", description: "高速穿线，迫使防线覆盖入口和侧路；不掉落金币。" },
  ordinary: { name: "普通敌人", glyph: "●", maxHp: 108, speed: .019, coreDamage: 12, attack: 13, attackEvery: 10, reward: 0, weight: 1, className: "ordinary", description: "基础压力单位，阻挡权重 1；不掉落金币。" },
  caster: { name: "术师敌人", glyph: "◉", maxHp: 118, speed: .016, coreDamage: 16, attack: 17, attackEvery: 9, reward: 0, weight: 1, className: "caster", description: "推进中会远程攻击高台单位，接触后继续施压前排；不掉落金币。" },
  swarm: { name: "群体敌人", glyph: "••", maxHp: 62, speed: .025, coreDamage: 8, attack: 8, attackEvery: 7, reward: 0, weight: 1, className: "swift", description: "以数量消耗阻挡容量；不掉落金币。" },
  armored: { name: "重甲敌人", glyph: "▣", maxHp: 320, speed: .011, coreDamage: 24, attack: 21, attackEvery: 12, reward: 0, weight: 2, className: "armored", description: "阻挡重量 2，需要破甲或持续输出；不掉落金币。" },
  elite: { name: "异端先驱", glyph: "✦", maxHp: 560, speed: .014, coreDamage: 32, attack: 28, attackEvery: 10, reward: 5, weight: 2, className: "elite", description: "能远程压制高台并以重量 2 冲击前排；击杀奖励只结算一次。" },
  "cave-boss": { name: "洞穴之影", glyph: "◉", maxHp: 560, speed: .014, coreDamage: 32, attack: 28, attackEvery: 10, reward: 5, weight: 2, className: "cave-boss", description: "W5 小Boss；生命首次低于 50% 时触发转身之痛。" },
...
export const effectiveAttackRange = (piece: Piece) => {
  ...
  if (!unit) return 0;
  return Math.max(unit.combat.range * 9, unit.terrain === "ground" ? 10 : 0, isThroneSlot(piece.slotId) ? 22 : 0);
};
...
export const idleBattle = (): BattleState => ({
  status: "idle", phase: "preparation", tick: 0, gameTime: 0, spawnRemaining: [], spawned: 0,
...
export function startWave(state: GameState) {
  ...
  let snapshot = createTraitSnapshot(validPieces, preparationPlan);
  let pieces = validPieces;
  ...
  if (agendas.length) {
    ...
    snapshot = createTraitSnapshot(validPieces, preparationPlan);
  }
  if (agendas.includes("market")) gold = capGold(gold + 1);
  if (agendas.includes("education")) ({ level, xp } = normalizeProgress(level, xp + 2));
  if (agendas.includes("citizen")) pieces = pieces.map((piece) => ({ ...piece, shield: (piece.shield ?? 0) + Math.round((piece.maxHp ?? 0) * .08) }));
  ...
  const waveCheckpoint = {
    ...
    shop: [...state.shop], pieces: state.pieces.map((piece) => ({ ...piece })), preparationPlan: { ...state.preparationPlan, pendingResearchSelections: [...(state.preparationPlan.pendingResearchSelections ?? [])], activeResearches: state.preparationPlan.activeResearches?.map((research) => ({ ...research })), enlightenmentAgendas: [...(state.preparationPlan.enlightenmentAgendas ?? [])] },
  };
...
export function restartCurrentWave(state: GameState) {
  ...
  return {
    state: {
      ...state, gold: checkpoint.gold, level: checkpoint.level, xp: checkpoint.xp, coreHp: checkpoint.coreHp,
      shop: [...checkpoint.shop], pieces: checkpoint.pieces.filter((piece) => characterById[piece.characterId]).map((piece) => ({ ...piece })), preparationPlan: checkpoint.preparationPlan ?? state.preparationPlan,
      battle: { ...idleBattle(), lastEvent: `第 ${state.wave} 波已回到准备阶段。` },
    },
    message: `第 ${state.wave} 波已从波次开始处重开。`, ok: true,
  };
}
  ...
  return [...latched].sort((a, b) => b.progress - a.progress).map((enemy) => {
    ...
    const pieceCandidate = alivePieces
      .filter((piece) => {
        ...
        return distance(deploymentPoint(piece.slotId), point) < 10 && (used.get(piece.id) ?? 0) + enemy.weight <= capacity;
      })
    ...
    const structureCandidate = liveStructures.find((structure) => {
      ...
      return distance(structure.point, point) <= contactRadius && (used.get(`structure:${structure.id}`) ?? 0) + enemy.weight <= structure.capacity;
    });
...
export function advanceBattle(state: GameState): GameState {
  ...
  const triggerBossPhases = (target: Enemy) => {
    if ((target.kind !== "boss" && target.kind !== "cave-boss") || target.hp <= 0) return;
    const triggered = new Set(target.bossPhasesTriggered ?? []);
    const phases = target.kind === "cave-boss" ? [CAVE_SHADOW_PHASE] : ABSOLUTE_SPIRIT_PHASES;
  ...
  const recordSmallSynergy = (profile: EffectProfile, target: Enemy) => {
    ...
    if (snapshot.smallSynergyTiers.dialectic >= 2 && ["socrates", "plato", "fichte", "hegel"].includes(source.characterId)) {
      if ((target.contradictionImmuneUntil ?? 0) <= gameTime) {
        ...
        target.contradiction = Math.min(3, stacks + 1); target.contradictionExpiresAt = gameTime + 6;
        if (target.contradiction >= 3) {
          ...
          if (snapshot.smallSynergyTiers.dialectic >= 3 && gameTime >= dialecticEnergyCooldownUntil) {
            ...
            if (recipient) queue.enqueue({ id: `dialectic-energy-${tick}-${recipient.id}`, kind: "energy", sourceId: source.id, targetKind: "ally", targetId: recipient.id, amount: 12, copyable: false });
            dialecticEnergyCooldownUntil = gameTime + 4;
          }
      ...
      while (pending.length) {
        ...
        if (profile.targetKind === "enemy") {
          ...
          if (profile.kind === "damage") {
            ...
            const source = profile.sourceId ? pieces.find((piece) => piece.id === profile.sourceId) : undefined; const logicBonus = source?.characterId === "russell" && snapshot.smallSynergyTiers["logical-analysis"] >= 2 ? 1 : 0; const count = Math.min(4, Math.max(2, Math.round(profile.amount ?? ((source?.star ?? 1) + 1)) + logicBonus));
            ...
            const atoms = Array.from({ length: count }, (_, index): Enemy => ({
              ...mother, id: `${mother.id}-atom-${index + 1}`, kind: "ordinary", hp: mother.hp / count, maxHp: mother.maxHp / count,
              ...
              sourceRouteId: mother.sourceRouteId ?? mother.lane, progress: Math.max(0, mother.progress - index * .001), evidence: undefined,
            }));
  ...
  const spawnRemaining = [...battle.spawnRemaining]; let spawned = battle.spawned ?? 0;
  const definition = waveDefinition(state.wave);
  const spawnInterval = definition.spawnInterval;
  if (spawnRemaining.length && tick % spawnInterval === 1) {
    const kind = spawnRemaining.shift()!; const template = enemyTemplates[kind]; const multiplier = definition.healthMultiplier; const lane = routeForSpawn(spawned);
    enemies.push({ id: `w${state.wave}-${tick}`, kind, hp: Math.round(template.maxHp * multiplier), maxHp: Math.round(template.maxHp * multiplier), progress: 0, lane, sourceRouteId: lane, weight: template.weight, shield: 0, energy: 0, maxEnergy: 100, rewardValue: template.reward, coreDamageValue: template.coreDamage, bossPhasesTriggered: kind === "boss" || kind === "cave-boss" ? [] : undefined }); spawned += 1; statistics.enemiesSpawned += 1; routeStatistics(lane).spawned += 1;
  }
  ...
  const castSkill = (piece: Piece, options: { derivedEffect?: boolean; powerScale?: number } = {}) => {
    ...
    if (unit.skill.id === "socratic" && primary) {
      ...
      candidates.forEach((enemy) => {
        const duration = enemy.kind === "boss" ? 1 : [0, 1.5, 1.8, 2.1][piece.star];
      ...
      const target = enemies.filter((enemy) => enemy.hp > 0 && (enemy.kind === "boss" || enemy.kind === "cave-boss" || enemy.kind === "elite")).sort((a, b) => b.progress - a.progress || b.hp - a.hp)[0] ?? highestHp(targets); if (target) { skillTarget = target; const duration = target.kind === "boss" ? 1.68 : target.kind === "cave-boss" || target.kind === "elite" ? 3.36 : 1.6; statusManager.add({ id: `thing-itself-${tick}`, targetId: target.id, sourceId: piece.id, kind: "silence", startedAt: gameTime, expiresAt: gameTime + duration, potency: 1, derivedEffect }); target.energy = 0; queue.enqueue({ id: `kant-delayed-${tick}`, kind: "damage", sourceId: piece.id, targetKind: "enemy", targetId: target.id, amount: Math.min(180, unit.combat.damage * 1.5 * power), position: routePoint(target.progress, target.lane), radius: 16, executeAt: gameTime + duration, derivedEffect, copyable: true }); mechanicBonus = 1; target.sealedTicks = Math.max(target.sealedTicks ?? 0, target.kind === "boss" ? 7 : 14); target.delayedDamage = Math.min(180, Math.round(unit.combat.damage * 1.5 * power)); }
      ...
      const special = primary.kind === "caster" || primary.kind === "elite" || primary.kind === "cave-boss" || primary.kind === "boss";
  ...
  for (const sourceId of snapshot.unitIds) {
    ...
    const baseRate = (live.attackSpeedTicks ?? 0) > 0 ? Math.max(3, unit.combat.attackEvery - 3) : unit.combat.attackEvery; const rate = Math.max(1, Math.round(baseRate / researchAttackSpeedMultiplier));
  ...
  let status: BattleStatus = "running"; let phase: CombatPhase = "combat"; let wave = state.wave; let gold = state.gold; let level = state.level; let xp = state.xp; let experienceGained = battle.experienceGained ?? 0; let summary: BattleSummary | undefined; let lastEvent = phaseThisTick ? `${enemyTemplates.boss.name}进入阶段：${phaseThisTick.name}` : `第 ${state.wave} 波推进中`;
  if (coreHp <= 0) {
    ...
    status = "defeat"; phase = "settlement"; lastEvent = "绝对精神突破防线，本局挑战失败。";
    summary = { wave, kills, coreDamage, killGold: Math.round(goldEarned), baseIncome: 0, interest: 0, perfectBonus: 0, totalGold: 0, elapsedTicks: tick, success: false, experienceGained: battle.experienceGained ?? 0, synergyTriggers: { ...synergyTriggers }, bossPhases: [...bossPhaseLog], statistics };
  } else if (!spawnRemaining.length && !enemies.length) {
    const { baseIncome, interest, perfectBonus } = settlementIncome(battle.lockedInterest ?? 0, coreDamage === 0); const potentialGold = Math.round(goldEarned) + baseIncome + interest + perfectBonus; const settledGold = capGold(gold + potentialGold); const totalGold = settledGold - gold; gold = settledGold;
    ...
    summary = { wave, kills, coreDamage, killGold: Math.round(goldEarned), baseIncome, interest, perfectBonus, totalGold, elapsedTicks: tick, success: true, experienceGained, synergyTriggers: { ...synergyTriggers }, bossPhases: [...bossPhaseLog], statistics };
    phase = "settlement";
  ...
  return { ...state, pieces, gold, level, xp, wave, coreHp, shop, preparationPlan, campaignElapsedSeconds, balanceHistory, waveEconomy, battle: nextBattle };
}
