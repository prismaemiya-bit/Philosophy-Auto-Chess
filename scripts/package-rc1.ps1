...
$name = "idea-garrison-v0.1-demo-rc1-portable"
$target = Join-Path $releaseRoot $name
$packageZip = Join-Path $releaseRoot "$name.zip"
$sourceZip = Join-Path $releaseRoot "idea-garrison-v0.1-demo-rc1-source.zip"

foreach ($path in @($target, $packageZip, $sourceZip)) {
  if (Test-Path -LiteralPath $path) { throw "RC1 artifact already exists and will not be overwritten: $path" }
}
if (-not (Test-Path -LiteralPath (Join-Path $root "dist\server\index.js"))) {
  throw "Production build missing. Run npm.cmd run build first."
}
...
Copy-Item -LiteralPath (Join-Path $root "RELEASE_NOTES_v0.1-demo-rc1.md") -Destination $target

$forbidden = Get-ChildItem -LiteralPath $target -Recurse -Force | Where-Object {
  $_.FullName -match "(User Data|Local Storage|IndexedDB|idea-garrison-v01-save|\.log$|\.pid$)"
}
...
$staging = Join-Path ([IO.Path]::GetTempPath()) "idea-garrison-v0.1-demo-rc1-source"
...
$sourceItems = @(
  ...
  "RELEASE_NOTES_v0.1-demo-rc1.md"
)
...
Set-Content -LiteralPath (Join-Path $releaseRoot "v0.1-demo-rc1-SHA256SUMS.txt") -Value $artifactHashes -Encoding utf8

Write-Host "Portable RC1: $target"
Write-Host "Package ZIP: $packageZip"
Write-Host "Frozen source snapshot: $sourceZip"
